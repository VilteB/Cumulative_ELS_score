...
# EW: without imputation:
# based on this dataset: ELS_PCM

# rename age variable (needs to be adjusted for each time point)
ELS_PCM$age_child = ELS_PCM$age_child.23y


fit_is <- lm(intern_score_z ~ prenatal_stress + postnatal_stress + prenatal_stress*postnatal_stress + 
                         sex + age_child, data = ELS_PCM)
...
fit_is_full <- lm(intern_score_z ~ prenatal_stress + postnatal_stress + prenatal_stress*postnatal_stress + 
                            sex + age_child + m_bmi_before_pregnancy + m_smoking + m_drinking, data = ELS_PCM)
mod2 <- summary(fit_is_full)
...
fit_fm <- lm(fat_mass_z ~ prenatal_stress + postnatal_stress + prenatal_stress*postnatal_stress + 
                         sex + age_child, data = ELS_PCM)
mod3 <- summary(fit_fm)
...
fit_fm_full <- lm(fat_mass_z ~ prenatal_stress + postnatal_stress + prenatal_stress*postnatal_stress + 
                              sex + age_child + m_bmi_before_pregnancy + m_smoking + m_drinking, data=ELS_PCM)
mod4 <- summary(fit_fm_full)
...
fit_grp <- multinom(risk_groups ~ prenatal_stress + postnatal_stress + prenatal_stress*postnatal_stress + 
                                sex + age_child, model = T, data = ELS_PCM)
mod5 <- summary(fit_grp)
...
fit_grp_full <- multinom(risk_groups ~ prenatal_stress + postnatal_stress + prenatal_stress*postnatal_stress + 
                                     sex + age_child + m_bmi_before_pregnancy + m_smoking + m_drinking, model = T, data = ELS_PCM)

mod6 <- summary(fit_grp_full)
...
# ask Serena: typos again for pre_personal_stress + pre_interpersonal_stress?

fit_grp_dm <- multinom(risk_groups ~ pre_life_events + pre_contextual_risk + pre_parental_risks + pre_interpersonal_risks +
                                   post_life_events + post_contextual_risk + post_parental_risk + post_interpersonal_risk + post_direct_victimization +
                                   sex + age_child, model = T, data = ELS_PCM)
mod7 <- summary(fit_grp_dm)
...
fit_grp_dm <- multinom(risk_groups ~ pre_life_events + pre_contextual_risk + pre_parental_risks + pre_interpersonal_risks +
                                   post_life_events + post_contextual_risk + post_parental_risk + post_interpersonal_risk + post_direct_victimization +
                                   sex + age_child + m_bmi_before_pregnancy, + m_drinking + m_smoking, model = T, data = ELS_PCM)
mod8 <- summary(fit_grp_dm)
...
# ALSPAC work-around (needed when not imputing, I guess)
sink("../Results.23y.txt")
modls
sink()

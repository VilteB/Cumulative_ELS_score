...
alspac.table = alspac.table[!duplicated(alspac.table$cidB2957),]

# remote siblings (all qlet=B)
alspac.table=alspac.table[!alspac.table$qlet=="B",]
...
Prenatal_LifeEvents <- data.frame(alspac.table[,c("cidB2957", # add mothers ID here (find the equivalent term within ALSPAC)
...
Prenatal_Parental_NoSubUse <- data.frame(alspac.table[,c( "cidB2957",# add mothers ID here,
...
Prenatal_MH_LifestyleRisks_NoMI <- data.frame(alspac.table[,c("cidB2957", # add mothers ID here,
...
prenatal_stress <- Reduce(function(x,y) merge(x = x, y = y, by = 'cidB2957',  all.x = TRUE), 
...
# 
#renatal_stress <- merge(child_id, prenatal_stress, by = 'IDM', all.x = T) 
# 
# ####################################################################################################################################################
